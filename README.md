...
* **データ出力**: 成績一覧を CSV 形式（UTF-8 BOM付き）でダウンロードし、外部の成績管理システムに活用できます。
...
* **多言語対応 (i18n)**: 日本語および英語に対応しており、設定に応じてDBファイルやUIテキストが自動で切り替わります。
* **データ永続化**: 回答結果や設定内容はすべてバックエンドのデータベース（`database.py` 経由）に安全に保存されます。
